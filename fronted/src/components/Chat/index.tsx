...
import { batch, createEffect, createSignal, onMount ,onCleanup ,JSX } from "solid-js"
...
export default function () {
  ...
  const closeButtonlessModal: JSX.EventHandler<Node, Event> = e => {
    ...
    if (currentParent && currentParent.nodeName === 'DIALOG') {
      ...
      currentDialog.close();
      const iframe = document.getElementById('iframe-tips') as HTMLIFrameElement;
      iframe.src = "/tips.html"
    } else {
      throw new Error('Unable to find Dialog parent of custom button.');
    }
  };
  const handleHashChange = () => {
    if (window.location.hash === '#tips' || window.location.hash === '#func') {
      openModal();
      setTimeout(() => {
        const iframe = document.getElementById('iframe-tips') as HTMLIFrameElement;
        const iframeDoc = iframe.contentDocument || iframe.contentWindow?.document;
        if (iframeDoc) {
          const element = iframeDoc.querySelector(window.location.hash);
          if (element) {
  ...
  return (
    <main ref={containerRef!} class="mt-4">
      ...
      >
        <iframe id="iframe-tips" src="/tips.html" />
